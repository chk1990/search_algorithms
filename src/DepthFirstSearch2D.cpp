 ...
 */
template<typename T>
DepthFirstSearch2D<T>::DepthFirstSearch2D(const std::string& filename) : SearchBase2D<T>(filename, "dfs"),
 ...
 */
template<typename T>
void DepthFirstSearch2D<T>::findPath(const Point2D<T>& start)
{
    // Find path
    // =============================================
    ...
    std::stack<pathElement>* s = this->stack.get();
    ...
    while(!s->empty()) {
        ...
        const pathElement elem = stack->top();
        stack->pop();
...
/**
 * @brief Adds a point to the stack
